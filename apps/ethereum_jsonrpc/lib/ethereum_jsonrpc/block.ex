defmodule EthereumJSONRPC.Block do
  ...
  defp entry_to_elixir({key, quantity})
       when key in ~w(difficulty gasLimit gasUsed minimumGasPrice number size cumulativeDifficulty totalDifficulty paidFees epoch) and
  ...
  defp entry_to_elixir({key, _} = entry)
       when key in ~w(author extraData hash logsBloom miner mixHash nonce parentHash receiptsRoot sealFields sha3Uncles lastCommits slashes
